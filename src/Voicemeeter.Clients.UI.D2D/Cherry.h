#pragma once

#include "Voicemeeter.Clients.UI.Cherry/Builder.h"
#include "Voicemeeter.UI.D2D/Scene.h"

namespace Voicemeeter {
	namespace Clients {
		namespace UI {
			namespace D2D {
				class Cherry : public UI::Cherry::Builder<Adapters::Multiclient::Cherry, ::Voicemeeter::UI::D2D::Graphics::Instrumentation> {
					using Builder = UI::Cherry::Builder<Adapters::Multiclient::Cherry, ::Voicemeeter::UI::D2D::Graphics::Instrumentation>;

				public:
					inline Cherry(
						HWND hWnd,
						::Environment::IInputTracker& inputTracker,
						::Environment::ITimer& compositionTimer,
						Adapters::Multiclient::Cherry& mixer
					) : Builder{ inputTracker, compositionTimer, mixer }
					  , m_hWnd{ hWnd } {

					};
					Cherry() = delete;
					Cherry(const Cherry&) = delete;
					Cherry(Cherry&&) = delete;

					inline ~Cherry() = default;

					Cherry& operator=(const Cherry&) = delete;
					Cherry& operator=(Cherry&&) = delete;

					::std::unique_ptr<::Voicemeeter::UI::D2D::Scene> Build();

				private:
					HWND m_hWnd;
				};
			}
		}
	}
}