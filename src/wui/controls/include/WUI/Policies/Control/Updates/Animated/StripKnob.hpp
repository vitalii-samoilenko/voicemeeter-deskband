#ifndef WUI_POLICIES_CONTROL_UPDATES_ANIMATED_STRIPKNOB_HPP
#define WUI_POLICIES_CONTROL_UPDATES_ANIMATED_STRIPKNOB_HPP
					...
					public:
						...
						inline void operator()(TStripKnob *that, state_t const &state) const {
							...
							size_t gain{
								...
								+ pop(floor(absl(state.degree * 4 / 15)))
							};
							if (state.toggle) {
					...
					public:
						...
						inline void operator()(TStripKnob *that) const {
							...
							that->set_Label(shifted < 0
								? context.target
								: context.gain);
							sub(&targetRgba, 3) = sub(targetRgba, 3) * absl(shifted)
								/ (_StripKnob_AnimationLength / 2);
